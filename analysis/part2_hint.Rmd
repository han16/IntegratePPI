...
#### HINT NETWORK (shortest paths matrix) ####

path="C:\\han\\Dataset\\20241114_HINT_ProteinDatabase\\"

hint.dat = as_tibble(read.table(paste(path, "HomoSapiens_cocomp_hq.txt", sep=""), fill = TRUE, header=T))  # use co-complex data as it has the largest number of edges 
hint.dat=hint.dat %>% select(Gene_A, Gene_B, high_quality)


## keep interactions with high_quality
hint.keep = hint.dat %>% filter(high_quality=="True")
hint.keep= hint.keep[!duplicated(hint.keep), ]  # remove duplicates https://stackoverflow.com/questions/26043121/r-removing-backwards-duplicate-rows-to-construct-an-edgematrix
hint.graph = ftM2graphNEL(as.matrix(hint.keep[,1:2]))
hint.path = johnson.all.pairs.sp(hint.graph)  # the step numbers of shortest path between gene pairs 

#save(hint.keep, hint.graph, hint.path, file="C:\\han\\Projects\\IntegratePPI\\tmp01_HINT_graph.Rdata")
```
...
```{r, message=F, warning=F, eval=T}
load("C:\\han\\Projects\\IntegratePPI\\tmp01_HINT_graph.Rdata")
...
top = unique( top[ top %in% rownames(hint.path) ] )

## pos are top genes also in HINT
pos = rownames(hint.path)[ rownames(hint.path) %in% top ]
pos = unique(pos)
pos = pos[ !is.na(pos) ]
rn = rownames(hint.path)[ rownames(hint.path) %in% rownames(bs) ]
