...
find_gene=function(x)
{
  if (length(which(sfari.id$Protein.stable.ID==x))>0)
    return(sfari.id$Gene.name[which(sfari.id$Protein.stable.ID==x)])
 if (length(which(sfari.id$Protein.stable.ID==x))==0)
   return("NA")
  
}
protein1_gene=unlist(sapply(string.keep$protein1, find_gene))
                     
protein2_gene=unlist(sapply(string.keep$protein2, function(x) sfari.id$Gene.name[which(sfari.id$Protein.stable.ID==x)]))
